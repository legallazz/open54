class ReadingSettings {
  ...
  calculatePages() {
    ...
    if (window.bookReader) {
      window.bookReader.totalPages = Math.ceil(contentHeight / viewportHeight);
      if (window.bookReader.updatePageInfo) {
        window.bookReader.updatePageInfo();
      }
    }
  }

  showCurrentPage() {
    if (window.bookReader && window.bookReader.showCurrentPage) {
      window.bookReader.showCurrentPage();
    }
  }
  ...
  saveReadingProgress(progressPercent) {
    const bookId = window.bookConfig?.getCurrentBook?.()?.id || "unknown-book";
